...
def split_ids(len_ids):
    train_size = int(round((95 / 100) * len_ids))
    # valid_size = int(round((10 / 100) * len_ids))
    test_size = int(round((5 / 100) * len_ids))
    # print(len_ids)
    ...
    test_indices = list(range(len_ids))

    # print("train_indices"+str(len(train_indices)))
    # print("test_indices"+str(len(test_indices)))
...
def get_dataloaders(input_paths, target_paths, batch_size):
    #input_paths=input_paths[0:100]
    #target_paths=target_paths[0:100]
    ...
    transform_target = transforms.Compose(
        [transforms.ToTensor(), transforms.Resize((352, 352), antialias=True), transforms.Grayscale()]
    )
