...
def build(args):
    ...
    if args.test_dataset == "Kvasir":
        ...
        target_paths = sorted(glob.glob(depth_path))
    elif args.test_dataset == "chest":
        img_path = args.root + "imagesTs/*"
        input_paths = sorted(glob.glob(img_path))
        depth_path = args.root + "labelsTs/*"
        target_paths = sorted(glob.glob(depth_path))
    elif args.test_dataset == "siim":
        img_path = args.root + "imagesTs/*"
        input_paths = sorted(glob.glob(img_path))
        depth_path = args.root + "labelsTs/*"
        target_paths = sorted(glob.glob(depth_path))
    # print("input_paths"+str(len(input_paths)))
    # print("target_paths"+str(len(target_paths)))
...
def predict(args):
    ...
    for i, (data, target) in enumerate(test_dataloader):
        ...
        input_path = target_paths[i].replace("labelsTs", "imagesTs")
        input_image = cv2.imread(input_path) # (1024,1024,3)
        predicted_map = cv2.resize(predicted_map.astype(np.uint8), 
...
def get_args():
    ...
    parser.add_argument(
        "--train-dataset", type=str, required=True, choices=["Kvasir", "CVC", "chest", "siim"]
    )
    parser.add_argument(
        "--test-dataset", type=str, required=True, choices=["Kvasir", "CVC", "chest", "siim"]
    )
